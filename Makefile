...
${TARGETDIR}/distri: ${TMPDIR} FORCE
	cargo zigbuild --profile ${PROFILE} --target ${DEFAULT_CONTAINER_TARGET}.${CONTAINER_GLIBC} -p distri --bin distri 
...
build-linux-arm: ${TMPDIR} FORCE
	cargo zigbuild --profile ${PROFILE} --target ${LINUX_ARM_TARGET}.${CONTAINER_GLIBC} -p distri --bin distri 

build-mac: ${TMPDIR} FORCE
	cargo build --profile ${PROFILE} -p distri --bin distri 

build-mac-intel:  ${TMPDIR} FORCE
	cargo build --profile ${PROFILE} --target ${MAC_INTEL_TARGET} -p distri --bin distri 

use crate::{Distri, DistriConfig, LlmExecuteResponse, ProviderType, TtsSpeechRequest};
    ...
    let resp = client
        .tts_speech(
            TtsSpeechRequest::new("Test")
                .with_model("tts-1-hd")
                .with_voice("nova")
                .with_provider(ProviderType::Azure),
        )
        .await
        .unwrap();
...
fn tts_speech_request_builder() {
    ...
    assert_eq!(req.provider, Some(ProviderType::OpenAI));

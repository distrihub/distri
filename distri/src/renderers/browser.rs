use crate::printer::{COLOR_CYAN, COLOR_GRAY, COLOR_RESET};
use distri_types::{Part, ToolResponse};

/// Render browsr_scrape / browsr_crawl results — typically markdown content.
pub fn render_scrape(result: &ToolResponse) {
    for part in &result.parts {
        match part {
            Part::Data(value) => {
                if let Some(obj) = value.as_object() {
                    // Show URL if present
                    if let Some(url) = obj.get("url").and_then(|u| u.as_str()) {
                        println!("{}  scraped: {}{}", COLOR_CYAN, url, COLOR_RESET);
                    }
                    // Show markdown preview
                    if let Some(markdown) = obj.get("markdown").and_then(|m| m.as_str()) {
                        let lines: Vec<&str> = markdown
                            .lines()
                            .filter(|l| !l.trim().is_empty())
                            .take(3)
                            .collect();
                        for line in &lines {
                            println!("{}  {}{}", COLOR_GRAY, line, COLOR_RESET);
                        }
                        let total = markdown.lines().filter(|l| !l.trim().is_empty()).count();
                        if total > 3 {
                            println!("{}  … ({} lines total){}", COLOR_GRAY, total, COLOR_RESET);
                        }
                    } else if let Some(title) = obj.get("title").and_then(|t| t.as_str()) {
                        println!("{}  {}{}", COLOR_GRAY, title, COLOR_RESET);
                    }
                } else {
                    crate::renderers::data::render_data_compact(value);
                }
            }
            Part::Text(text) => {
                let lines: Vec<&str> = text.lines().take(3).collect();
                println!("{}  {}{}", COLOR_GRAY, lines.join("\n  "), COLOR_RESET);
            }
            _ => {}
        }
    }
}

/// Render browsr_browser / browser_step results — action confirmations.
pub fn render_browser_step(result: &ToolResponse) {
    for part in &result.parts {
        match part {
            Part::Text(text) => {
                // Browser step results are usually short status messages
                let lines: Vec<&str> = text.lines().take(3).collect();
                println!("{}  {}{}", COLOR_GRAY, lines.join("\n  "), COLOR_RESET);
            }
            Part::Data(value) => {
                if let Some(obj) = value.as_object() {
                    if let Some(url) = obj.get("url").and_then(|u| u.as_str()) {
                        println!("{}  url: {}{}", COLOR_GRAY, url, COLOR_RESET);
                    }
                    if let Some(status) = obj.get("status").and_then(|s| s.as_str()) {
                        println!("{}  status: {}{}", COLOR_GRAY, status, COLOR_RESET);
                    }
                }
            }
            Part::Image(file) => {
                let label = match file {
                    distri_types::FileType::Bytes { name, .. } => {
                        name.as_deref().unwrap_or("screenshot").to_string()
                    }
                    distri_types::FileType::Url { url, .. } => url.clone(),
                };
                println!("{}  screenshot: {}{}", COLOR_GRAY, label, COLOR_RESET);
            }
            _ => {}
        }
    }
}

...
impl CodeParsingHooks {
    pub fn new(tools_registry: Arc<LlmToolsRegistry>) -> Self {
        Self {
            tools_registry,
...
"#);
        ...
        for tool in self.tools_registry.tools.values() {
            ...
            let description = def.function.description.as_deref().unwrap_or("No description");
...
impl AgentHooks for CodeParsingHooks {
    async fn llm_messages(&self, messages: &[Message]) -> Result<Vec<Message>, AgentError> {
        info!("🔧 CodeParsingHooks: Modifying system prompt to include code execution instructions");

...
impl AgentFactoryRegistry {
    pub fn new() -> Self {
        ...
        registry.register_factory("standard".to_string(), Arc::new(|definition, tools_registry, executor, context, session_store| {
    ...
    pub fn create_agent(
        ...
        let factory = self
            .factories
            .get(agent_type)
            .ok_or_else(|| AgentError::NotFound(format!("Agent factory for type '{}' not found", agent_type)))?;

        Ok(factory(definition, tools_registry, executor, context, session_store))
    }

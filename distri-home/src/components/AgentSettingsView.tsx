...
import type { Secret, ProviderSecretDefinition, CustomModelEntry, CustomProviderConfig } from '../DistriHomeClient';
...
export function AgentSettingsView({ className }: AgentSettingsViewProps) {
  ...
  const loadData = useCallback(async () => {
    ...
    setError(null);
    try {
      const [settings, secs, defs] = await Promise.all([
  ...
  return (
    <div className={`space-y-6 ${className ?? ''}`}>
      ...
      {customProviders.map((cp) => {
        ...
        // Generate key definitions for custom providers
        const keys = [
      ...
      <div className="rounded-xl border border-dashed border-border/70 bg-card/50 shadow-sm overflow-hidden">
        {showAddProvider ? (
          <div className="px-6 py-4 space-y-3">
            ...
            <div>
              ...
              <input
                ...
                placeholder="e.g. LangDB, My Ollama, Company LLM"
                className="w-full rounded-lg border border-border/70 bg-background px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground/40 focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary"
              />
            </div>
            <div>
            ...
            <div>
              ...
              <input
                type="text"
                value={newProviderProjectId}
                onChange={(e) => setNewProviderProjectId(e.target.value)}
            ...
            <button
              ...
              disabled={!newProviderName.trim() || !newProviderUrl.trim() || !newProviderKey.trim() || saving}
          ...
          <button
            ...
            className="flex w-full items-center justify-center gap-2 px-6 py-4 text-sm text-muted-foreground transition hover:text-foreground"
          >

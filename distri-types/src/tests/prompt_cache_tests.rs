...
use crate::prompt::{
    PromptRegistry, TemplateData, build_prompt_messages_with_budget, rough_token_count,
};
...
async fn section_cache_hit() {
    ...
    let (c1, t1) = registry
        .render_section_cached("test", "{{instructions}}", &data)
        .await
        .unwrap();
    let (c2, t2) = registry
        .render_section_cached("test", "{{instructions}}", &data)
...
async fn section_cache_miss_after_invalidation() {
    ...
    let (c1, _) = registry
        .render_section_cached("k", "{{instructions}}", &data)
        .await
        .unwrap();
    ...
    let (c2, _) = registry
        .render_section_cached("k", "{{instructions}}", &data2)
        .await
        .unwrap();
...
async fn clear_section_cache_wipes_all() {
    ...
    registry
        .render_section_cached("a", "{{instructions}}", &data)
        .await
        .unwrap();

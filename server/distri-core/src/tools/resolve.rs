    ...
    fn test_oauth_connection(id: uuid::Uuid, provider: &str) -> Connection {
        Connection {
            ...
            auth: ConnectionAuth::Oauth {
                provider: distri_types::connections::OAuthProviderConfig {
                    name: provider.to_string(),
                    display_name: None,
                    authorization_url: format!("https://example.com/{provider}/authorize"),
                    token_url: format!("https://example.com/{provider}/token"),
                    refresh_url: None,

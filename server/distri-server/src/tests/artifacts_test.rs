...
mod tests {
    ...
    async fn create_test_filesystem() -> (Arc<dyn FileSystemOps>, TempDir) {
        ...
        use distri_filesystem::{FileSystemConfig, create_file_system};
    ...
    async fn test_list_artifacts_checks_both_thread_and_task_levels() {
        ...
        let thread_namespace = distri_types::ArtifactNamespace::new(
        ...
        let thread_wrapper = ArtifactWrapper::new(
            filesystem.clone(),
            thread_path.clone(),
        )
        .await
        ...
        thread_wrapper
            .save_artifact("thread_artifact.json", r#"{"type": "thread", "data": "from_thread"}"#)
            .await
            .expect("Failed to save thread artifact");
        
        // Create artifacts in task-level path
        let task_wrapper = ArtifactWrapper::new(
            filesystem.clone(),
            task_path.clone(),
        )
        .await
        ...
        task_wrapper
            .save_artifact("task_artifact.json", r#"{"type": "task", "data": "from_task"}"#)
            .await
        ...
        assert_eq!(paths_to_check.len(), 2, "Should have both thread and task paths");
        
        // Check each path
        for path_id in paths_to_check {
            let wrapper = ArtifactWrapper::new(
                filesystem.clone(),
                path_id.clone(),
            )
            .await
        ...
        assert_eq!(all_artifacts.len(), 2, "Should find artifacts from both thread and task levels");
        assert!(all_artifacts.contains(&"thread_artifact.json".to_string()), "Should find thread artifact");
        assert!(all_artifacts.contains(&"task_artifact.json".to_string()), "Should find task artifact");
    }
    ...
    async fn test_list_artifacts_handles_duplicate_filenames() {
        ...
        let duplicate_count = all_artifacts.iter()
            .filter(|&name| name == duplicate_filename)
            .count();
        assert_eq!(duplicate_count, 1, "Should only return duplicate filename once");
    }
}
